...
<AccordionGroup>
  <Accordion title="Channel subpaths">
    ...
    | `plugin-sdk/channel-ingress` | Experimental channel ingress access graph helpers for state resolution, decision composition, typed gate lookup, redacted access-facts projection, turn-admission mapping, and simple string or multi-identifier subject/adapter construction. Use only from migrated channel runtime paths. See [Channel ingress API](/plugins/sdk-channel-ingress). |
